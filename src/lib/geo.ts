 ...
 *
 * Trimmed port of the web console's `src/lib/geo.ts`. The coordinate data is
 * copied verbatim — same landmass, same six zone polygons, same 21 towns — so
 * a pin sits in the same place on the phone as it does in the browser. Nothing
 * is fetched: the map renders identically offline.
 *
 * The only difference is the viewBox: mobile uses a smaller, portrait-friendly
 * user space. MAP_SIZE keeps the exact aspect ratio of MAP_BOUNDS
 * (1.06° lng × 1.32° lat) so the projection introduces no distortion.
 */
...
/** SVG viewBox the projection maps into — 1060 × 1320, exactly bounds-proportional. */
export const MAP_SIZE = { width: 1060, height: 1320 } as const

/** Alias kept for parity with the web module's export name. */
export const MAP_VIEWBOX = MAP_SIZE

/** Ready-made `viewBox` string for <Svg>. */
export const MAP_VIEWBOX_STR = `0 0 ${MAP_SIZE.width} ${MAP_SIZE.height}`
...
/** Inverse of {@link project} — used for tap-to-place / readouts. */
export function unproject(x: number, y: number): { lat: number; lng: number } {
  const { minLng, maxLng, minLat, maxLat } = MAP_BOUNDS
  const lng = minLng + (x / MAP_SIZE.width) * (maxLng - minLng)
  const lat = maxLat - (y / MAP_SIZE.height) * (maxLat - minLat)
  return { lat, lng }
}
...
/** Look a town up by name. */
export function townByName(name: string): TownMarker | undefined {
  return TOWNS.find((t) => t.name === name)
}

/** Great-circle distance in kilometres — powers the proximity readout. */
